...
const comparisons = '=><';
const variables = {
	// It's like I don't even care at this point
	'const ': true,
	'let ': true,
	'var ': true
};
...
const replacements = {
	// literally the dumbest code I have ever written
...
function highlightCode(prompt, code, comment) {
	...
	for (let line of lines) {
		for (const [original, replacement] of Object.entries(replacements)) {
			// Like why don't I just make this garbage procedure into a function???
		...
		while (position < line.length) {
			// Who cares about a dynamic sized tokenizer thingy!!!!
		...
		generated.push(
			<pre key={uuidv4()} className="tab">
				{generatedLines}
			</pre>
		); // Not even gonna say anything...s
		generatedLines = [];
	}
